...
la integración continua y los comandos necesarios para reproducir el trabajo.
...
Por último, y con el fin de facilitar la reproducción del trabajo, se recogen los
comandos principales. El detalle de uso de la interfaz se encuentra en los
Anexos~\ref{anexo::web} a~\ref{anexo::docker-ghcr}.

\begin{verbatim}
# Preprocesado (una sola vez): pre-redimensionado del dataset a 224x224
python -m src.preprocess_resize

# Entrenamiento de un modelo (arquitectura y configuracion de clases)
python -m src.main --model densenet121 --class-config nofracture12

# Evaluacion sobre el test silver-standard
python -m src.evaluate

# Intervalos de confianza bootstrap del leaderboard
python -m src.bootstrap_ci --n-boot 2000 --seed 42

# Arranque de la API REST y de la interfaz web
